...
import countryCodesFakerFactory from "./countryCodesFakerFactory";
...
const CountryCodesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = countryCodesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("countryCodes").create(elem));
...
export default connect(mapState, mapDispatch)(CountryCodesFakerDialogComponent);
