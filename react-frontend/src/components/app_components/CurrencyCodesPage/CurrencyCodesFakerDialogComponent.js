...
import currencyCodesFakerFactory from "./currencyCodesFakerFactory";
...
const CurrencyCodesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = currencyCodesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("currencyCodes").create(elem));
...
export default connect(mapState, mapDispatch)(CurrencyCodesFakerDialogComponent);
