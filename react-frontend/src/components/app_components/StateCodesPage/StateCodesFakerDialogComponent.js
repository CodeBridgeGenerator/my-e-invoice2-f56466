...
import stateCodesFakerFactory from "./stateCodesFakerFactory";
...
const StateCodesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stateCodesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stateCodes").create(elem));
...
export default connect(mapState, mapDispatch)(StateCodesFakerDialogComponent);
