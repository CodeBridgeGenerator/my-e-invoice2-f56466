...
const ProductsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [measurement], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            productName: _entity?.productName,quantity: _entity?.quantity,unitPrice: _entity?.unitPrice,measurement: _entity?.measurement?._id,
    ...
    useEffect(() => {
                    ...
                    client
                        .service("measurements")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleMeasurementsId } })
                        .then((res) => {
