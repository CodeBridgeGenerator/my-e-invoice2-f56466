...
import measurementsFakerFactory from "./measurementsFakerFactory";
...
const MeasurementsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = measurementsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("measurements").create(elem));
...
export default connect(mapState, mapDispatch)(MeasurementsFakerDialogComponent);
