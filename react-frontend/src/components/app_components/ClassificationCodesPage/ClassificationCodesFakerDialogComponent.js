...
import classificationCodesFakerFactory from "./classificationCodesFakerFactory";
...
const ClassificationCodesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = classificationCodesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("classificationCodes").create(elem));
...
export default connect(mapState, mapDispatch)(ClassificationCodesFakerDialogComponent);
