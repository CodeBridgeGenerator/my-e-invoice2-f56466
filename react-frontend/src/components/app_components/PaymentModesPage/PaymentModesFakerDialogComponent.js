...
import paymentModesFakerFactory from "./paymentModesFakerFactory";
...
const PaymentModesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = paymentModesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("paymentModes").create(elem));
...
export default connect(mapState, mapDispatch)(PaymentModesFakerDialogComponent);
