...
import taxTypesFakerFactory from "./taxTypesFakerFactory";
...
const TaxTypesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = taxTypesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("taxTypes").create(elem));
...
export default connect(mapState, mapDispatch)(TaxTypesFakerDialogComponent);
